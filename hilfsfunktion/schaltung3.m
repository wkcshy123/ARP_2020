function [i_F, wirkungsgrad_getriebe] = schaltung3(Fahrzeug, Rad, Geschwindigkeit,T_Bedarf)
i_G = Fahrzeug.i_Get;
Gang = 1;
...
for i=1:length(Geschwindigkeit.Data)
    ...
    if motordrehzahl >= 2200 && ((0 <= motordrehmoment) && (motordrehmoment <= 750))                                                 
        if Gang ~= length(i_G)
            Gang = Gang + 1;
        end
    elseif motordrehzahl <= 1000 && motordrehmoment >= 1100                                         
        if Gang >= 3
            Gang = Gang - 2;
